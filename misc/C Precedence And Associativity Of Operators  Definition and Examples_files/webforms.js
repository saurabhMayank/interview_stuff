(function() {
    if (typeof window.ml_jQuery !== 'undefined' && typeof window.ml_jQuery.ajax === 'function') {
        ml_load_libs();
        return;
    }
    ...
    function ml_webform_embed() {
        ...
        ml_jQuery('.ml-subscribe-form form, .ml-contact-form form').each(function () {

            var $form = ml_jQuery(this);
            var $wrapper = ml_jQuery(this).closest('.ml-subscribe-form, .ml-contact-form');
            ...
            if ($form.data('ml-submit-bound') === undefined || !$form.data('ml-submit-bound')) {
                ...
                $wrapper.find('.ml-block-success').bind('click', function () {
                    ...
                    var $checkboxes = $wrapper.find('.ml-block-form').find('input[type="checkbox"],input[type="radio"]');
                ...
                $form.bind('submit', function (e) {
                    ...
                    if (ml_validate_form($form)) {
                        ...
                        if (!preview || form_data.toLowerCase().indexOf('email') >= 0) {
                            ml_jQuery.ajax({
                                type: 'GET',
                                url: form_url,
                                data: form_data,
                        ...
                        if ($form.data('close') && typeof window[closeFnName] === 'function') {
                            window[closeFnName]();
                        }
                        ...
                        if ($form.data('redirect')) {
                            ...
                            if (preview) {
                                window.top.open($form.data('redirect'),  target || '_blank');
                            } else {
                                window.parent.postMessage('mlWebformRedirect-' + encodeURIComponent($form.data('redirect')) + (target ? ('-' + target) : ''), "*");
                            }
                        }
                    }
                });
    ...
    function ml_validate_form($form) {
        ...
        $form.find('.ml-validate-required').each(function (index, w) {
            ...
            ml_jQuery(w).find('input[type="text"], input[type="email"], select, textarea').each(function (i, v) {
                if (ml_jQuery(v).val() !== undefined && ml_jQuery(v).val() !== '') {
                    element_valid = true;
                }
            });
            ml_jQuery(w).find('input[type="checkbox"],input[type="radio"]').each(function (i, v) {
